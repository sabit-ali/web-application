...
export default function SearchDialog() {
  ...
  useEffect(() => {
    if (query) {
      (async () => {
        const value = query
        await axios.get('/api/v1/thread/getthreads', {
          params: {
            searchString: value
          },
              ...
              <>
                {data?.map((fields: any) => (
                  <div key={fields?._id} className=''>
                    <Link to={`/onethread/${fields._id}`} className=' flex gap-4 justify-center items-center '>
                    <img src={fields?.avatar} alt={fields?.title} className=' h-12 w-12 rounded-full object-cover' />
